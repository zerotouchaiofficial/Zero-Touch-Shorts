# ================================================================
# 🎬 YouTube Shorts Generator — No Duplicate Facts + Thumbnail
# ================================================================
...
# ── Used-facts store (persisted in repo root) ─────────────────────
...
def fetch_and_generate(out_dir):
    ...
    seen_this_run = set()
    log.info(f'📚 {len(used_ever)} facts already used globally — skipping')
    ...
    while total < 50 and fails < 60:
        try:
            ...
            k = f.lower().replace(' ', '')
            # Skip if used in this run OR any previous run ever
        ...
        try:
            ...
            fails += 1; time.sleep(0.5)

    # Persist all used facts so next run skips them
...
def mix_audio(apaths, fstarts, total, out_dir):
    ...
    ((music - 14).overlay(tts_t).overlay(dng_t - 3)).export(
        mix_path, format='mp3')
...
def download_backgrounds(total, out_dir):
    ...
    # Gradient fallback
    if not ipaths:
        log.warning('No Unsplash images — using gradients')
...
def draw_progress(draw, t, total):
    ...
    if fx > x1 + 5:
        draw.rounded_rectangle([x1,by,fx,by+10], radius=5,
                               fill=(255,220,0,200))
    draw.ellipse([fx-7, by-3, fx+7, by+13],
...
def draw_top(draw, t, total):
    ...
    if t < IDUR:
        a = int(255 * (min(t/0.35, 1.0) if t < IDUR-0.5
                       else (IDUR-t)/0.5))
        a = max(0, min(255, a))
...
def render_video(facts, durs, wtimes, fstarts, total,
    ...
    def render(t):
        ...
        for i in range(len(facts)):
            if fstarts[i] <= t < fstarts[i] + durs[i]:
                draw_karaoke(dr, t, i, facts, fstarts,
                             durs, wtimes)
                break
        draw_top(dr, t, total)
...
def generate_thumbnail(facts, ipaths, out_dir):
    ...
    # Pick most vibrant background image
    ...
    for y in range(520):
        a = int(230*(1-y/520)**1.3)
        dg.line([0,y,W,y], fill=(5,0,25,a))
    ...
    F_BADGE = load_font(52)
    F_DYK1  = load_font(128)
    F_DYK2  = load_font(172)
    F_SUB   = load_font(62)
    ...
    # Dark backing plate so title ALWAYS readable
    CY    = H//2 - 80
    pt1   = CY - 230
    pt2   = CY + 230
    ...
    # "DID YOU KNOW?"
    glow_text(dt, (W//2, CY-110),
              'DID  YOU', F_DYK1,
              (255,255,255,255), (120,180,255), gr=20)
    glow_text(dt, (W//2, CY+80),
              'KNOW?', F_DYK2,
